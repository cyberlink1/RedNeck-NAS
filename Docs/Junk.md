...
* The frontend has been refactored so that all custom CSS lives in `assets/css/style.css` and JavaScript is now modular.  Shared helpers are located in `assets/js/functions.js` and each view gets its own script (`disks.js`, `raid.js`, `lvm.js`, `mounts.js`, `nfs.js`, `samba.js`).  Pages no longer contain inline `<style>` blocks or `<script>` tags with executable code; common helpers such as the confirmation modal handler and initial‑message display are centralized in `functions.js`.  New disk‑view actions (partition deletion, wipe) also use the shared confirmation dialog.
* A dark mode toggle has been added to the top navigation bar (and the login screen) so you can switch themes before or after authentication.  When enabled the UI uses a set of high‑contrast green/black colours (the values listed in `test.md`).  The body element is given the classes `dark-mode bg-main` so the overall page background is `--bg-main`; the `.container` also uses `--bg-main` and each card/panel uses `--bg-panel`.  All tables are rendered with a `--bg-panel` background, headers and cell borders coloured `--header-green`, and text in `--text-main`.  The header/menu bar becomes `--header-green` with a bottom border of `--header-border`, and all text within the header (including buttons/links) is shown in `--text-muted` for consistency.  The close (×) icons used in modals/alerts are also recoloured by overriding the Bootstrap SVG background to use `currentColor`, then setting the button colour to `--accent` under dark mode so they go green instead of black.
* Secondary (grey) buttons across the site now use the `--terminal-green` background with `--bg-main` text for improved visibility.
* Primary (blue) buttons have been inverted: background set to `--text-muted` with `--bg-main` text color to keep the subdued palette.
* Dark mode control is now a simple Bootstrap “form-switch” slider instead of an icon button; the switch state persists in localStorage.  The preference is saved in `localStorage` so it persists across pages.
* A full‑screen spinner overlay (`#spinnerOverlay`) is defined once in `dashboard.php` (so it’s available on every view) and controlled via JavaScript.  It remains hidden by default and is shown automatically whenever any form is submitted, giving visual feedback during long‑running operations like formatting or extending volume groups.  The overlay will no longer appear before confirmation dialogs, and the LV modal is reopened automatically after any format/remove operation so you won’t lose context.  The spinner is also displayed during disk and RAID format/wipe operations (including the partition‑format modal) so you always get feedback when the backend is working.  For those AJAX‑driven actions the code now hides the spinner when the response returns or if an error occurs, preventing the interface from locking up indefinitely.
...
* Any device (including MD arrays) that already contains a filesystem is also excluded from the LVM "Disks & Physical Volumes" table.  This avoids offering formatted disks such as `/dev/md0` for PV initialization.  Arrays that only have RAID metadata (`FSTYPE=linux_raid_member`) are still treated as raw and will appear in the list when unpartitioned; if you partition an MD device the base `/dev/mdN` is hidden and the individual partitions (`/dev/mdNpX`) are offered instead.  The disk scan now understands both `/dev/md0` and the alternative `/dev/md/0` naming used on some systems.


- **Dashboard**: This is now the single entry point; each menu choice is implemented as a "view" loaded under `dashboard.php` (e.g. `dashboard.php?view=lvm`, `?view=nfs`, `?view=mounts`, `?view=disks`).  The `views/` directory contains the PHP fragments for each feature, and the former standalone pages (`lvm.php`, `nfs.php`, `mounts.php`, `disks.php`) simply redirect here and are considered deprecated.  You can still hit them and will be forwarded automatically.  When first logging in (or when no `view=` parameter is supplied) the dashboard displays a row of statistic cards: total physical drives (excluding the OS/root disk), total RAID arrays, total logical volumes, total `/export` mounts, and total NFS exports.  The mount count is derived directly from `/proc/1/mounts` (with a fallback to `nsenter`+`mount|grep`), ensuring the number reflects the host’s actual mount table even when the PHP process is confined to its own namespace.  Each card shows a large count number to give an at-a-glance summary.  (Previously only logical volumes and mounts were shown.)  The mounts card still uses `col-md-auto` and accompanying CSS so its width expands as needed to accommodate the output; this prevents a horizontal scrollbar inside the card (note that the overall page may scroll if the card becomes wider than the viewport).  The Mounts view itself now presents an explicit table of existing `/export` mounts (device, point and current options), with an easy **Create mount** button in the top‑right corner; unmount actions are available per row.  Confirmation prompts for both mount and unmount operations inject a hidden field containing the action name before submitting, ensuring the backend sees `mount_lv`/`umount_lv` even though the standard `form.submit()` call doesn’t include the clicked button.  On a successful unmount the backend now also strips any matching entry from `/etc/fstab` (via sudo) so the share won’t reappear after reboot.  The PHP code loads the file itself, uses a regexp to drop any line whose second whitespace‑separated field equals the mountpoint (this handles leading spaces), then writes the filtered lines back via `sudo -n tee /etc/fstab.tmp` and then moves the file into place; make sure `mv` is included in the sudoers entry (it is required for this cleanup).  The unmount response will now simply report the result of the umount command and note when an fstab entry was removed.  Additional debugging output is no longer shown to the user.  The status message will note when an fstab line was removed.  To keep the interface on the correct page, every mount-related form also includes a hidden `view=mounts` field and the dashboard script honours the `view` parameter from POST as well as GET.  Clicking any row (outside of the unmount button) will open an **Edit mount** dialog pre‑populated with the current options and fstab status, allowing you to tweak `rw/ro/noexec/nosuid/nodev` flags or toggle the “mount at boot” checkbox.  The mount-at-boot checkbox is now honoured during initial creation as well as edits; the backend correctly recognises a successful mount and adds the entry to `/etc/fstab`.  Unmount operations also clear any matching fstab line once the point is gone (the code now correctly detects disappearance from the mount table and cleans the file).  The creation form opens a modal in which the **Device** pulldown lists any block node (physical drive, `/dev/md*`, or LV) that currently has a filesystem and isn’t already mounted.  The list explicitly excludes the OS root disk/partition, any device that’s already an LVM physical volume (whether or not it’s part of a VG), and — importantly — **individual disks/partitions that are members of an MD array**; only the array device itself is offered.  This prevents you from accidently mounting a member which has no usable filesystem while hiding the correct `/dev/mdX` target.  You can also specify filesystem options (rw, ro, noexec, nosuid, nodev) and there’s a **Mount at boot** checkbox – checking it will append an appropriate line to `/etc/fstab` (the backend uses `sudo -n tee -a /etc/fstab`, so `/usr/bin/tee` must be permitted in sudoers) using the detected filesystem type.  The navbar now reads RAID, LVM, Mounts, NFS Exports, Disks; select the appropriate section (RAID and LVM are split into separate views).  Note that the LV must contain a valid filesystem before mounting – format using the LVM view first if necessary.  The installer now includes `util-linux` so that `nsenter` is available; without it mount/umount requests may be confined to Apache’s private namespace.
...
- **Disks**: A new management section lists physical devices in a table; clicking a row opens a pop‑up showing its partition table and status.  The table now includes a **Used by** column which indicates the MD array(s) a disk belongs to or the volume group owning it when it's an LVM PV, plus a new **SMART** column on the right that shows the one-line health result obtained via `smartctl -H`.  If a disk has an existing partition table the column will show **Partition** so you can tell at a glance that the device isn’t completely empty.  MD array devices themselves are labelled “Raid” in both the **Status** and **Used by** columns so they remain obvious.  You can create/delete partitions, wipe a disk (wiping a device that was part of a removed RAID may trigger loud GPT header warnings from sgdisk; these are filtered out and treated as a success), check SMART (the SMART button now pops up a modal showing the full `smartctl -a` report, rendered in a `<pre>` so the raw text is preserved exactly as it would appear in a terminal; any trailing `(exit N)` status lines are silently dropped), or format a partition via secondary dialogs.  When deleting an entire RAID array the backend now performs several safety steps: it verifies the array isn’t still used by LVM, runs `wipefs -a` on the `/dev/mdX` node to clear any filesystem signatures, stops the array and then removes it before zeroing superblocks on the member devices.  This prevents stale signatures from causing the array to reappear in other views.  Modals now consistently include both **OK** and **Cancel** buttons (the delete‑partition dialog and the post‑operation “result” notice previously lacked a cancel control).  The shared `showConfirmation()` helper always shows the Cancel button regardless of whether a callback was provided; earlier it hid the control when only an OK handler was passed, causing unmount warnings to lack a cancel option.  All interactions occur via AJAX and use centralized confirmation/result modals; the interface hides destructive buttons when the device is in use or is a CD/DVD.  (See the code and earlier paragraphs for full implementation details.)
...
- **LVM**: View and manage physical volumes, volume groups, and logical volumes. The upper‑left pane now shows a single table combining uninitialised disks and existing PVs; each row includes device path, model (RAID devices are labelled “Raid Device”), volume group and size, and disks not yet allocated include a checkbox for creating a new PV. A **Volume groups** button at the top of the page opens a modal window listing all VGs (name, size, free space); the first column contains checkboxes so you can select one or more groups. A single **Extend** button sits between the Create and Remove controls; clicking it when one or more groups are checked opens a dialog showing each chosen VG alongside the pool of unused physical volumes. Tick the PVs you want to add under each group and submit – the server loops through each VG and runs `vgextend`. When you click **Remove Group**, a confirmation modal enumerates the selected VGs and warns that data will be lost; accepting submits the removal. The backend now leaves the underlying physical volumes intact (their LVM metadata is preserved), so they remain visible in the top table and may be assigned to another group later.  Within the VG modal you can also launch a dialog to create a new VG.
  ...
  A new **Logical volumes** button sits beside the VG control. Clicking it reveals a modal containing the same information as the standalone logical‑volumes card; action buttons for create, format and remove appear in the footer next to the Close button. The **Create** button spawns a second modal with the familiar create‑LV form. **Format** now presents a dialog listing all selected volumes (checkboxes are provided in the LV table) and offers a filesystem dropdown populated automatically from `/sbin/mkfs.*`; choose the type and submit to format all checked LVs. **Remove** has been improved: you can check multiple volumes and the remove dialog will operate on all of them (the modal shows the list and the confirmation message reflects the count) rather than prompting separately for each one. The implementation now always submits a set of hidden `lvs[]` inputs representing the selected volumes, so the backend reliably receives every choice even if the dropdown is hidden/disabled. All sub‑dialogs confirm before submitting. The list of LVs updates on reload and the parent modal is restored after you close any sub‑dialog.
...
- **NFS**: List current exports in a compact table showing only the export directory and the underlying device.  (The device column is now blank whenever the directory is not mounted from a local block device – for example when the host’s root filesystem itself comes from NFS – the code uses `findmnt`/`df` and will ignore non‑`/dev/*` sources.  On a stock Debian installation `/etc/nfs.conf` does not include a `root` setting by default.)  Click any row to open an **Edit export** dialog which displays all clients/options for that export; you can add, modify or delete individual client entries from within the dialog, and a new **Delete** button lets you remove the entire export line without returning to the table.  A separate **Create export** button above the table launches the familiar add‑export modal where you may enter an optional comment and select a directory before building a new client list.  The “Client entry” modal has been widened so that the many option fields do not spill outside the dialog.  In both the creation form and the **Edit export** dialog the clients/options are chosen via a mixture of radio buttons and drop‑down selectors so mutually exclusive flags cannot be combined.  You select `rw`/`ro` from a dropdown, then choose one of the squash modes, one of sync/async, subtree‑check vs no‑subtree‑check and wdelay vs no‑wdelay from dropdowns; remaining boolean flags (`noaccess`, `crossmnt`, `nohide`) stay as checkboxes, and numeric fields permit anonuid/anongid/fsid.  When an export is created or edited the UI constructs a valid `/etc/exports` line and the backend updates the file and runs `sudo -n exportfs -ra` so the NFS server reloads its configuration.  When an export line is removed the script now also strips the comment line that appears immediately *above* it (if any), ensuring you don’t end up with orphaned comments after deleting the last client for a directory.      
