...
- **Disks**: New disk management menu. The view now presents devices in a full‑width table rather than a dropdown. Each row shows the device path, its model/serial "name", size, and a status column indicating RAID/LVM membership (it will list the specific `/dev/mdN` array when present) or if it’s the OS disk. Non‑disk entries such as CD‑ROMs, loops and partitions are omitted (MD arrays are included); devices named `/dev/sr*` are filtered out entirely since they can’t be partitioned. Click a row to see a pop‑up containing the partition table and status for that disk.  Action buttons (Create/Delete Partition, wipe, SMART, identify) are now shown in a single row in the modal footer alongside the Close button instead of in separate cards.  The Create Partition button is disabled once the disk has four partitions; attempting to submit a new size when four already exist will return a warning message.  The data is loaded via AJAX directly from `views/disks.php?ajax=1&disk=<device>`; this bypasses the dashboard wrapper so only the card HTML is returned.  The preview appears in a **separate info modal** (id `infoModal`) so it doesn’t interfere with the global confirmation modal used for destructive actions.  All action forms within the modal are submitted via AJAX and the returned HTML replaces the modal contents; the server-side handler detects `ajax=1` in either GET or POST requests and returns **only** the card fragment, so the disks table (or any other surrounding page HTML) never appears in the popup.  Partition creation and deletion are now handled in *secondary* modals rather than inline cards.  Clicking the **Create Partition** or **Delete Partition** buttons on the disk details popup will hide the preview and open a smaller dialog where you specify the size (for creation) or, in the case of deletion, select the partition number from a dynamically populated dropdown.  The latter queries the server for the current partition list so you don’t have to remember the numbers.  These submodals submit via the same AJAX helper, hide themselves on completion, and then refresh the main disk details view; confirmation prompts are still shown for destructive operations.  Unlike earlier versions, the creation form now instructs `parted` to start the new partition at 1 MiB rather than 0 %, avoiding the “requested a partition from 0.00B…” error that appeared briefly.  When subsequent partitions are created we parse `parted print` to calculate the end of the previous partition and start one MiB later.  The form also inspects the remaining space (via `lsblk`) and will refuse sizes larger than what’s available, warning "Requested size (…) exceeds available space (…)".  If the requested size can be parsed the code computes an explicit end offset in MiB (start + size) and passes that to `parted`; this avoids subtle rounding or interpretation differences that previously produced "end before start" errors.  Users may still type values such as `1G`, `500M` or `2T`; the start used for partitioning is always in MiB but the size string is only used for computation, not passed verbatim.  This simplifies the UI while still allowing flexible size specification.  Confirmation dialogs are shown only after any open info modal has fully hidden, guaranteeing they stack correctly and avoiding the “behind the preview”/greyed‑out backdrop problem.  The script also reuses the same Bootstrap modal instance when refreshing the contents; this prevents multiple backdrop layers from building up when you perform several actions in succession.  When creating a partition the handler will automatically initialise a GPT label on unlabelled disks to avoid the “unrecognised disk label” error.  The preview modal does not reload the page on close (forms inside still submit normally); use the Close button when done.
