...
- **Disks**: New disk management menu. The dropdown lists all block devices except CD‐ROMs (type `rom`) and loop/partition entries; MD arrays (`/dev/md*`) are included too. Select a device to view its size/model/serial and examine the partition table. You can create or delete partitions using `parted` syntax (start/end may be percentages or size units) and wipe the disk (GPT table and any filesystem superblocks via `sgdisk --zap-all` and `wipefs`), but these actions are disabled if the device is a member of an MD array, is itself an MD device, or is already used as an LVM PV. SMART health/temperature queries and identify/locate signals remain available regardless. Confirmation dialogs protect destructive operations.
